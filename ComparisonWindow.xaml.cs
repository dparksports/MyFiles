...
namespace FileLister
{
    public partial class ComparisonWindow : Window
    {
        ...
        private HashSet<string> LoadPaths(string file)
        {
            ...
            // Regex to parse CSV lines, handling quoted fields containing commas
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (line.StartsWith("\""))
                {
                    // Quoted CSV: parse and unquote
                    var parts = csvRegex.Split(line);
                    path = parts[0].Trim('"').Trim();
                }
                else
                {
                    // Unquoted line: assume entire line is path (even if it has commas)
                    path = line.Trim();
                }
